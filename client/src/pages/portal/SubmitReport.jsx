...
import { FileText, Clock, AlertCircle, CheckCircle, Calendar, ChevronRight, Edit2, Lock } from "lucide-react";
...
const WeekModal = ({ onSelect, onClose, submittedTypes }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl shadow-2xl p-6 animate-slide-up">
        <h2 className="text-xl font-bold text-gray-900 dark:text-slate-100 mb-1">Which week is this report for?</h2>
        ...
        <div className="space-y-3 mb-6">
          ...
          <div className="p-4 border-2 border-amber-200 dark:border-amber-800 rounded-xl">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-gray-900 dark:text-slate-100">Previous week (arrears)</p>
                <p className="text-xs text-gray-500 dark:text-slate-400">
...
const SubmitReport = () => {
  ...
  const fetchSubmitted = async () => {
    try {
      const { reports } = await getMyReports({ weekType: "current", status: "submitted" });
  ...
  const handleTypeClick = (type) => {
    ...
    if (existing && !editMode) {
      // Already submitted this week for this type - show options
      setPendingType(type);
      setShowWeekModal(false);
      setSelectedType(null);
      // Go straight to edit mode for this report
      setWeekType("current");
    ...
    <div className="space-y-6 animate-fade-in">
      {showWeekModal && (
        <WeekModal
          ...
          onClose={() => { setShowWeekModal(false); setPendingType(null); }}
          submittedTypes={submittedThisWeek.map((r) => r.reportType)}
        />
      )}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="section-subtitle">Drafts save automatically. Submit during the open window.</p>
        </div>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <FileText className={cn("w-5 h-5", submitted ? "text-green-600 dark:text-green-400" : "text-purple-600 dark:text-purple-400")} />
                    {submitted ? (
                      ...
                      <span className="text-xs text-amber-600 dark:text-amber-400">Draft only</span>
                    ) : null}
                  </div>
                  <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">{type.label}</p>
      ...
      {selectedType && FormComponent && (
        <div>
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            <button onClick={handleBack} className="btn-ghost text-sm py-1.5">Back</button>
            <h2 className="font-bold text-gray-900 dark:text-slate-100">
              {REPORT_TYPES.find((t) => t.value === selectedType)?.label}
            </h2>
            {weekType === "late" ? (
              <span className="badge-warning">
                Arrears: {lateWeekDate ? new Date(lateWeekDate).toDateString() : "Previous week"}
              </span>
            ) : (
              <span className="badge-success">Current week</span>
            )}
            {editMode && <span className="badge-info flex items-center gap-1"><Edit2 className="w-3 h-3" /> Editing</span>}
            {!portal?.isOpen && <span className="badge-warning">Draft mode</span>}
          </div>
          <FormComponent
            weekType={weekType || "current"}
            portalOpen={portal?.isOpen}
            lateWeekDate={weekType === "late" ? lateWeekDate : null}
