...
const TIMING_LABELS = {
  "early-60plus": { label: "60+ early",  color: "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400" },
  "early-30to60": { label: "30-60 min",  color: "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400" },
  "early-15to30": { label: "15-30 min",  color: "bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400" },
  "early-0to15":  { label: "0-15 min",   color: "bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400" },
  "late":         { label: "Late",        color: "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400" },
};
...
const AttendanceHistory = () => {
  ...
  const fetchDetails = async (sessionId) => {
    ...
    try {
      ...
      setExpandedAttendance((p) => ({ ...p, [sessionId]: data }));
      setExpandedId(sessionId);
    } catch { toast.error("Error", "Could not load session details."); }
    finally { setLoadingDetails((p) => ({ ...p, [sessionId]: false })); }
  };
  ...
  const downloadCSV = (sessionId) => {
    ...
    const rows = [["Name", "Worker ID", "Department", "Check-in Time", "Timing", "On Duty", "Verified"]];
    attendance.forEach((a) => {
      rows.push([
        a.worker?.fullName, a.worker?.workerId, a.worker?.department?.replace(/-/g, " "),
    ...
    a.href = url;
    const date = new Date(session.serviceDate).toLocaleDateString("en-GH").replace(/\//g, "-");
    a.download = `attendance-${session.serviceType}-${date}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const timeStr = (iso) => iso ? new Date(iso).toLocaleTimeString("en-GH", { hour: "2-digit", minute: "2-digit" }) : "N/A";
  const dateStr = (iso) => iso ? new Date(iso).toLocaleDateString("en-GH", { weekday: "short", day: "numeric", month: "short", year: "numeric" }) : "N/A";
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      {loading ? <Loader text="Loading attendance history..." /> : sessions.length === 0 ? (
        <div className="card p-12 text-center">
          <Calendar className="w-12 h-12 text-gray-300 dark:text-slate-600 mx-auto mb-4" />
          <p className="font-semibold text-gray-900 dark:text-slate-100 mb-2">No sessions found</p>
          <p className="text-sm text-gray-500 dark:text-slate-400">Try adjusting the filters above.</p>
        </div>
      ) : (
          ...
          {sessions.map((session) => {
            ...
            return (
              <div key={session._id} className="card overflow-hidden">
                ...
                >
                  <div className={cn("w-2 h-12 rounded-full flex-shrink-0", session.isOpen ? "bg-green-400" : "bg-gray-200 dark:bg-slate-600")} />
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <p className="font-bold text-gray-900 dark:text-slate-100">{serviceLabel} Service</p>
                      {session.specialServiceName && <span className="text-xs text-gray-500 dark:text-slate-400">({session.specialServiceName})</span>}
                      {session.isOpen
                        ? <span className="text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-2 py-0.5 rounded-full font-medium">Live</span>
                        : <span className="text-xs bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400 px-2 py-0.5 rounded-full">{session.closedBy === "auto" ? "Auto-closed" : "Closed"}</span>
                      }
                    </div>
                  ...
                  <div className="flex items-center gap-4 flex-shrink-0">
                    <div className="text-right">
                      <p className="text-2xl font-bold text-purple-700 dark:text-purple-400">{stats.totalCheckedIn || 0}</p>
                      <p className="text-xs text-gray-400 dark:text-slate-500">checked in</p>
                    </div>
                    {stats.late > 0 && (
                      <div className="text-right">
                        <p className="text-lg font-bold text-red-600 dark:text-red-400">{stats.late}</p>
                ...
                {isExpanded && detail && (
                  <div className="border-t border-gray-100 dark:border-slate-700">
                    ...
                    <div className="px-5 py-3 border-t border-gray-50 dark:border-slate-800 flex justify-end">
                      <button onClick={() => downloadCSV(session._id)} className="btn-outline text-xs flex items-center gap-1.5">
                    ...
                    {detail.attendance?.length > 0 ? (
                      <div className="divide-y divide-gray-50 dark:divide-slate-800 max-h-80 overflow-y-auto">
                        {detail.attendance.map((a, i) => (
                          <div key={a._id} className="flex items-center gap-3 px-5 py-2.5 hover:bg-gray-50 dark:hover:bg-slate-800/30 transition-colors">
                            <span className="text-xs text-gray-300 dark:text-slate-600 w-5 flex-shrink-0 text-right">{i + 1}</span>
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                              <span className="font-mono text-sm text-gray-700 dark:text-slate-300">{timeStr(a.checkInTime)}</span>
                              <span className={cn("text-xs px-2 py-0.5 rounded-full font-medium", TIMING_LABELS[a.timingCategory]?.color)}>
                                {TIMING_LABELS[a.timingCategory]?.label}
                              </span>
                              {a.isOnDuty && <span className="text-xs bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300 px-2 py-0.5 rounded-full">Duty</span>}
                            </div>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="p-8 text-center text-sm text-gray-400 dark:text-slate-500">No attendance records for this session.</div>
                    )}
                  </div>
                )}
              </div>
